...
Furthermore, provide a registration screen. This screen should allow the user to register an account with a username, pin, and repeated pin. A warning should be shown that says "Please enter a pin" when a pin is missing, "pins do not match" when the pins do not match, and "a pin must be 7 digits" when it is not exactly 7 digits. This can either be done in real-time or upon clicking "register". Upon registration, if the username is already taken, inform the user. More specifically, for registration, check errors in the following order:

1. **Username empty:**
   If the username is empty, alert: **"You must provide a username!"**

2. **Missing PIN fields:**
   If either the PIN or the confirmation PIN is missing, alert:
   **"You must provide both a PIN and a confirmation PIN!"**

3. **PIN mismatch:**
   If the PIN and confirmation PIN do not match, alert:
   **"Your PIN and confirmation PIN do not match!"**

4. **Invalid PIN format:**
   If either PIN or confirmation PIN is not a valid 7-digit number, alert:
   **"PIN and confirmation PIN must both be 7-digit numbers."**

5. **Username already exists:**
   If the username is already registered, alert:
